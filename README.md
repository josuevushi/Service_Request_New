...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Dn.ServiceRequest.Web` and `Dn.ServiceRequest.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 5e6cd15d-4417-4329-a370-0847de62273c
```

> `5e6cd15d-4417-4329-a370-0847de62273c` is the password of the certificate, you can change it to any password you want.
...
Run `Dn.ServiceRequest.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `Dn.ServiceRequest.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `Dn.ServiceRequest.Web`: ASP.NET Core MVC / Razor Pages application that is the essential web application of the solution.
